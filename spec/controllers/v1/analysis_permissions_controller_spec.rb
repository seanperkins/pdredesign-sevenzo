...
describe V1::AnalysisPermissionsController do
  ...
  describe '#show' do
    ...
    context 'logged-in user' do
      ...
      before(:each) do
        FactoryGirl.create_list(:analysis_member, 2, :as_facilitator, analysis: inventory.analysis)
        FactoryGirl.create_list(:analysis_member, 3, :as_participant, analysis: inventory.analysis)
  ...
  describe '#update' do
    ...
    context 'logged-in user' do
      let(:inventory) { FactoryGirl.create(:inventory, :with_analysis) }
      let(:original_participant_users) { inventory.analysis.participants.map(&:user) }
      let(:original_participant_emails) { original_participant_users.map(&:email) }

      before(:each) do
        FactoryGirl.create_list(:analysis_member, 2, :as_participant, analysis: inventory.analysis)
        sign_in inventory.owner
        post :update, inventory_id: inventory.id, format: :json,
      ...
      it 'changed all roles' do 
        expect(inventory.analysis.facilitators.count).to be(2)
        expect(inventory.analysis.facilitators.map(&:user).map(&:email)).to include *original_participant_emails
      end
    end
  end
end
