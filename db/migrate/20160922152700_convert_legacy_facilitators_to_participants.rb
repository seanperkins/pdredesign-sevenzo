class ConvertLegacyFacilitatorsToParticipants < ActiveRecord::Migration
  def change
    ToolMember.where(tool_type: 'Assessment', role: ToolMember.member_roles[:facilitator]).where.not(user_id: ToolMember.where(tool_type: 'Assessment', role: ToolMember.member_roles[:participant]).select(:user_id))
        .each { |tool_member|
